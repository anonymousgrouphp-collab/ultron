...
| P0-C2 | Remove duplicate `organize_desktop` + duplicate shutdown tool | DEP: P0-A ✅signed | 🚫 | zcode-p0c 2026-09-07 | **blocked on `p0-crash-bugs` MERGE, not sign-off** (hotspot rule: `main.py` is P0-A's until it lands). Plan ready: drop `shutdown_jarvis` registry alias (`main.py:444` — undeclared in `tool_declarations.py`, model can't call it; only `shutdown_ultron` declared @ :316); dedupe `organize_desktop` — `file_controller.py:392` (declared path, `tool_declarations.py:191`) vs `desktop.py:297` (via `desktop_control` organize @ :209) — keep one impl + one path |
...
| P0-C4 | Name purge: JARVIS/HUNNY aliases → **ULTRON** only | DEP: P0-A ✅signed | 🚫 | zcode-p0c 2026-09-07 | **blocked on `p0-crash-bugs` MERGE** (same hotspot reason; edits `main.py`+`ui.py`). Plan ready: drop `JarvisUI` import+alias (`main.py:31`, `ui.py:672`) + `JarvisLive` alias (`main.py:1028`); default `_asst_name` "JARVIS"→"ULTRON" (`main.py:238-241`); `[JARVIS]` log tags (`main.py:983,997`); HUNNY docstrings (`core/installer.py:2`, `actions/system_monitor.py:152`). Do NOT rename the openWakeWord model file ("hey jarvis" = third-party artifact name) |
...
- 2026-09-07 (zcode-p0c): P0-C2/C4 are 🚫 on the `p0-crash-bugs` MERGE, not on sign-off (P0-A is ✅+signed) — AGENTS.md §3 reserves `main.py` for P0-A during Phase 0 until its merge, and both tasks edit it. Both rows carry exact execution plans; ~15 min of mechanical work once the merge lands. `p0-dead-code` is stacked on `p0-crash-bugs` tip 76623fa, so it fast-forwards after that merge.
...
- 2026-09-07: zcode-p0c — P0-C1 + P0-C3 done & signed on `p0-dead-code` (−1,598 LOC dead code); P0-C2/C4 → 🚫 merge-gated with plans in rows.
