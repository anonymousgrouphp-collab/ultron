"""kernel/voice/stt_stabilizer.py — flicker-free realtime partial text
(research adopt S2).

Ported from RealtimeSTT `core/realtime_text_stabilizer.py` (MIT, © 2023 Kolja
Beigel, cloned @ 07df360; report 11 §2). Repeated partial transcriptions of
the same utterance flicker: every decode revises earlier words, so naive
"latest text wins" displays stutter and any consumer acting on the partial
(routing, HUD) sees gibberish mid-revision. The stabilizer projects each
observation onto a normalized comparison text (NFKC, casefold, punctuation
stripped — with raw-offset mapping back), accumulates per-character evidence
across observations, and only publishes a character once it has enough
confirmations over a time span: spaces and punctuation need more, and a
stable right context. Outlier observations (similarity < threshold vs the
recent window) are quarantined and only adopted if the branch repeats.

Zero third-party dependencies — stdlib only. Consumed by the local voice loop
(`app/local_voice.py`); thresholds are configurable so a latency-biased
profile can trade stability for speed.
"""
...
class RealtimeTextStabilizationConfig:
    """Evidence thresholds for realtime text stabilization."""
...
class RealtimeTextObservation:
    """One realtime transcription observation (deterministic timestamps)."""
...
class RealtimeTextFinalObservation:
    """The completed transcription for reconciliation."""
...
class RealtimeTextEvidenceDiagnostics:
    """Evidence used to stabilize one text offset."""
...
class RealtimeTextObservationTiming:
    """Timing metadata for one realtime observation."""
...
class RealtimeTextStabilizationEvent:
    """The stabilizer result for one realtime observation."""
...
class RealtimeTextFinalizationEvent:
    """How the final transcription reconciles with stabilized text."""
...
class RealtimeTextStabilizationSnapshot:
    """Current stabilization state (diagnostics)."""
...
@dataclass(frozen=True)
class _Projection:
    """The normalized/raw offset relationship for one observation."""
...
class _EvidencePoint:
    """One accepted evidence observation for a text offset."""
...
class _ObservationRecord:
    """One realtime text observation and its projection metadata."""
...
class RealtimeTextStabilizer:
    ...
    def __init__(
        ...
        self._segment_id: Optional[Any] = None
        self._started_at_monotonic: Optional[float] = None
        self._started_at_wall_time: Optional[float] = None
    ...
    def observe(
        ...
        self._outlier_streak = []
        return self._accepted_event(record)
    ...
    def _ignored_event(
        ...
        stable_raw_end_offset = (
            self._stable_raw_end_offset(projection)
    ...
    def _confirmed_frontier(
        ...
        """Walks the comparison text while each character has stable evidence."""
    ...
    def _char_is_stable(self, offset: int, char: str, now: float) -> bool:
    ...
    def _evidence_for_offset(
        ...
        return RealtimeTextEvidenceDiagnostics(
            ...
            audio_start_sample_min=min(
                point.audio_start_sample for point in points
            ),
    ...
    def _build_delta(
        ...
        """Public delta for newly stable text (punctuation inserted at its
        raw position when itself stable)."""
        parts: List[str] = []
    ...
    def _outlier_decision(self, record: _ObservationRecord) -> str:
        """accept / adopt-branch / outlier, from similarity to recent text."""
...
def _project_text(text: str) -> _Projection:
    """Raw text → normalized comparison text with raw-offset mapping."""
    raw = text or ""
...
def _punctuation_before_offset(projection: _Projection, offset: int) -> str:
    if offset <= 0 or offset >= len(projection.raw_starts):
        return ""
    raw = projection.raw[
        projection.raw_ends[offset - 1]:projection.raw_starts[offset]
    ]
