"""kernel/voice/boundary.py — lightweight acoustic boundary detection
(research adopt S3).

Ported from RealtimeSTT `core/realtime_boundary_detector.py` (MIT, © 2023
Kolja Beigel, cloned @ 07df360; report 11 §3). Detects likely voiced-energy
valleys — vowel ends and vowel-to-pause transitions — from short PCM chunks
with nothing but numpy: a 10 ms log-energy envelope, a rolling noise floor,
an autocorrelation voicing score and zero-crossing rate. The local voice loop
uses these events to refresh partial transcriptions at natural pauses instead
of a blind timer, and they double as a cheap speech-onset hint for barge-in.

The detector is heuristic: it emits acoustic candidates, not certain
...
class SpeechBoundaryEvent:
    """One committed acoustic boundary candidate."""
    ...
    def __repr__(self) -> str:
        return (
            "SpeechBoundaryEvent(time={:.3f}s, score={:.2f}, reason={!r}, "
            "drop={:.1f}dB, valley={:.1f}dB)"
        ).format(
            self.boundary_time_seconds, self.score, self.reason,
            self.drop_db, self.valley_depth_db,
        )


class SpeechBoundaryResult:
    """Detector output for one processed chunk."""
...
class RealtimeSpeechBoundaryDetector:
    ...
    Builds a 10 ms log-energy envelope and looks for confirmed local energy
    valleys after voiced audio. A small lookahead (30 ms) is used so a
    ...
    def __init__(
        ...
        self.before_window_frames = max(
            self.min_voiced_frames + 2, int(round(180.0 / self.frame_ms))
        )
        ...
        self.recovery_db = (
            float(recovery_db) if recovery_db is not None else 2.2 - 1.1 * s
        )
        self.vowel_margin_db = (
            float(vowel_margin_db) if vowel_margin_db is not None else 14.0 - 5.0 * s
        )
        self.min_voicing_score = (
            float(min_voicing_score)
            if min_voicing_score is not None
            else 0.62 - 0.12 * s
        )
    ...
    def _analyze_frame(self, frame: np.ndarray) -> Dict[str, float]:
        ...
        self._processed_samples = start_sample + self.frame_samples
        self._frame_index += 1
    ...
    def _voicing_score(self, frame: np.ndarray, rms: float) -> float:
        """Autocorrelation peak over the 75-350 Hz pitch band (voiced speech
        is periodic; hiss is not)."""
    ...
    def _update_noise_floor(self, energy_db: float) -> None:
        """Dual-rate EMA: fast adaptation down to quiet floors, slow drift up."""
    ...
    def _maybe_detect_boundary(self) -> Optional[SpeechBoundaryEvent]:
        ...
        if (
            candidate["center_sample"] - self._last_boundary_sample
        ...
        vowel_frames = [
            frame for frame in recent_vowel_window if frame["is_vowel_like"]
        ]
        if len(vowel_frames) < self.min_vowel_frames:
            return None

        before_energies = np.array(
            [frame["energy_db"] for frame in before], dtype=np.float32
        )
        after_energies = np.array(
            [frame["energy_db"] for frame in after], dtype=np.float32
        )
        ...
        after_quiet = (
            avg_after < candidate["noise_floor_db"] + self.speech_margin_db + 2.0
        )
